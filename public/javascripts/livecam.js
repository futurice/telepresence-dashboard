...
function GstLiveCamServer(config) {
	...
	Assert.ok(OS.platform() == 'win32' || OS.platform() == 'darwin',
...
function LiveCam(config) {
	...
	var broadcast = function() {
		...
		gst_cam_process.stdout.on('data', function(data) {
			...
			if(
				data.toString().includes('Setting pipeline to PLAYING') > 0 ||
				data.toString().includes('Asetetaan liukuhihna tilaan SOI') > 0
			) {
